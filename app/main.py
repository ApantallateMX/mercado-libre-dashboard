...
@app.get("/api/product-net-analysis")
async def product_net_analysis(q: str = "", ads_pct: float = 5.0):
    """
    Busca un producto en los listings del seller actual y calcula el neto
    después de comisión MeLi, IVA comisión, ads y envío.
    q: texto de búsqueda (ej: 'hisense 58')
    ads_pct: porcentaje de ads sobre precio venta (default 5%)
    """
    client = await get_meli_client()
    if not client:
    ...
    try:
        COMMISSION = 0.17
        IVA_COMM   = 0.16
        ads_rate   = ads_pct / 100.0

        # 1. Buscar items del seller que coincidan
        search_resp = await client.get(
            f"/users/{client.user_id}/items/search",
            params={"search": q, "limit": 20}
        )
        item_ids = search_resp.get("results", [])
        if not item_ids:
            return {"query": q, "results": [], "message": "Sin resultados para la búsqueda"}

        # 2. Batch fetch items
        from app.services.meli_client import MeliClient
        batch_resp = await client.get("/items", params={"ids": ",".join(item_ids[:20])})
        items_raw = []
        if isinstance(batch_resp, list):
            items_raw = [e["body"] for e in batch_resp if e.get("code") == 200]
        elif isinstance(batch_resp, dict) and "body" in batch_resp:
            items_raw = [batch_resp["body"]]

        # 3. Para cada item, obtener deal price y calcular neto
        results = []
        for it in items_raw:
            price_reg  = it.get("price", 0)
            is_full    = it.get("shipping", {}).get("logistic_type") == "fulfillment"
            ...
            deal_price = None
            try:
                prices_resp = await client.get(f"/items/{it['id']}/prices")
                for p in prices_resp.get("prices", []):
                    if p.get("type") == "promotion" and p.get("conditions", {}).get("eligible"):
                        deal_price = p.get("amount")
                        break
            except Exception:
                pass

            sale_price = deal_price if deal_price else price_reg

            # Envío: TVs grandes con free_ship y merchant = ~$350 MXN para el seller
            ship_cost = 0
            if free_ship and not is_full:
                ship_cost = 350  # estimado TV 50-70"

            comm     = sale_price * COMMISSION
            iva_c    = comm * IVA_COMM
            ads      = sale_price * ads_rate
            total_d  = comm + iva_c + ads + ship_cost
            neto     = sale_price - total_d
            pct_ret  = (neto / sale_price * 100) if sale_price else 0
            ...
            results.append({
                ...
                "status":        status,
                "logistic":      "FULL" if is_full else "Merchant",
                "free_shipping": free_ship,
                "precio_regular": round(price_reg, 2),
                "precio_deal":    round(deal_price, 2) if deal_price else None,
                "precio_venta":   round(sale_price, 2),
                "deductions": {
                    "comision_17pct": round(comm, 2),
                    "iva_comision":   round(iva_c, 2),
                    f"ads_{int(ads_pct)}pct": round(ads, 2),
                    "envio_estimado": round(ship_cost, 2),
                    "total":          round(total_d, 2),
                },
                "neto_mxn":   round(neto, 2),
                "pct_retenido": round(100 - pct_ret, 1),
                "pct_recibido": round(pct_ret, 1),
            })

        return {"query": q, "ads_pct": ads_pct, "results": results}
    finally:
        await client.close()
