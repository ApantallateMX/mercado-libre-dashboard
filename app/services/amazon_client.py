...
async def _seed_amazon_accounts():
    ...
    if seller_id2 and refresh_rt2:
        ...
        # RESUELTO 2026-09-10 (ver .claude/memory/
        # project_amazon2_credential_fallback_verification_2026-08-31.md para el
        # diagnóstico original y DEVLOG.md 2026-09-10 para el cierre):
        #
        # Historial del problema — el refresh_token que AUTOBOT tenía guardado
        # (AMAZON2_REFRESH_TOKEN) había sido emitido bajo la app VeKtorClaude
        # (cuenta 1, VECKTOR) en vez de bajo su propia app "Claude Autobot
        # Dashboard" (app_solution_id 454ba70d-...). Un refresh_token de LWA
        # queda atado a la app que lo emitió, así que fallaba con
        # AMAZON2_CLIENT_ID/SECRET (HTTP 400 unauthorized_client) y solo
        # funcionaba con AMAZON_CLIENT_ID/SECRET de cuenta 1 — de ahí este
        # fallback `or client_id2`, mantenido a propósito mientras tanto (no
        # por descuido) para no romper AUTOBOT en producción.
        #
        # Causa raíz: app/auth.py (amazon_callback, rama _is_acct2) intercambiaba
        # el spapi_oauth_code de AUTOBOT usando siempre las credenciales de
        # cuenta 1. Corregido el 2026-08-31 (commit affe1e4) para usar
        # AMAZON2_CLIENT_ID/SECRET propias en ese intercambio.
        #
        # Jovan reautorizó AUTOBOT el 2026-09-10 con el fix de auth.py ya en
        # producción → nuevo AMAZON2_REFRESH_TOKEN emitido bajo la app correcta.
        # Verificado en vivo contra LWA (https://api.amazon.com/auth/o2/token,
        # grant_type=refresh_token) usando el refresh_token vigente en Railway +
        # AMAZON2_CLIENT_ID/SECRET (propias, sin fallback) → HTTP 200,
        # access_token emitido normalmente. El fallback ya no es necesario:
        # cada cuenta usa sus propias credenciales, como exige CLAUDE.md.
        await token_store.save_amazon_account(
            seller_id=seller_id2, nickname=nickname2,
            client_id=client_id2, client_secret=client_sec2,
