...
class BinManagerClient:
    ...
    async def get_bulk_stock_by_location(self, conditions: str = "GRA,GRB,GRC,NEW") -> list:
        """Retorna stock vendible (CONCEPTID=1) sumando LOC47 (CDMX) + LOC68 (MTY MAXX).

        Workaround para bug BM: LOCATIONID="47,62,68" en bulk siempre retorna 0 filas.
        Solución: 2 fetches paginados por separado, merge por SKU.
        Retorna mismos campos que get_bulk_stock(): SKU, TotalQty, Reserve, AvailableQTY,
        Brand, Model, Title, CategoryName, LastRetailPricePurchaseHistory, AvgCostQTY.
        """
        if not self._logged_in:
            if not await self.login():
                return []
        ...
        async def _fetch_loc(loc_id: str) -> list:
            base_payload = {
                "COMPANYID": 1, "SEARCH": "", "CONCEPTID": 1,
                "LOCATIONID": loc_id, "CONDITION": conditions,
                "FORINVENTORY": 0, "BUSCADOR": False, "RECORDSPAGE": _PAGE_SIZE,
                "NEEDAVGCOST": True, "NEEDRETAILPRICEPH": True,
                "CATEGORYID": None, "WAREHOUSEID": None, "BINID": None,
                "BRAND": None, "MODEL": None, "SIZE": None, "LCN": None,
                "OPENCELL": "", "OCCOMPTABILITY": "",
                ...
                for attempt in range(2):
                    try:
                        ...
                        if r.status_code == 200:
                            data = r.json()
                            page_rows = data if isinstance(data, list) else []
                            loc_rows.extend(page_rows)
                            if len(page_rows) < _PAGE_SIZE:
                                logger.info(f"[BM] bulk_by_loc LOC{loc_id}: {len(loc_rows)} SKUs en {page} págs")
                                return loc_rows
                            break
                        else:
                            return loc_rows
                    except Exception as e:
                        logger.error(f"BinManager bulk_by_loc LOC{loc_id} pág {page}: {e}")
                        if attempt == 0:
                            continue
                        return loc_rows
            return loc_rows

        rows_47 = await _fetch_loc("47")
        rows_68 = await _fetch_loc("68")
        logger.info(f"[BM] bulk_by_loc: LOC47={len(rows_47)}, LOC68={len(rows_68)} filas")

        if not rows_47 and not rows_68:
            logger.warning("[BM] bulk_by_loc: ambas ubicaciones retornaron vacío")
            return []

        # Merge por SKU: sumar TotalQty + Reserve; metadata de primera ocurrencia
        merged: dict = {}
        for row in rows_47 + rows_68:
            ...
            if sku not in merged:
                merged[sku] = {
                    "SKU": sku,
                    "TotalQty": 0, "Reserve": 0, "AvailableQTY": 0,
            ...
            merged[sku]["TotalQty"] += int(row.get("TotalQty") or 0)
            merged[sku]["Reserve"]  += int(row.get("Reserve")  or 0)
            if not merged[sku]["Brand"]:
                merged[sku]["Brand"] = row.get("Brand") or ""
            if not merged[sku]["CategoryName"]:
                merged[sku]["CategoryName"] = row.get("CategoryName") or ""
            if not merged[sku]["LastRetailPricePurchaseHistory"]:
                rph = float(row.get("LastRetailPricePurchaseHistory") or 0)
                if rph > 0:
                    merged[sku]["LastRetailPricePurchaseHistory"] = rph
            if not merged[sku]["AvgCostQTY"]:
                cost = float(row.get("AvgCostQTY") or 0)
                if cost > 0:
                    merged[sku]["AvgCostQTY"] = cost

        result = []
        for row in merged.values():
            row["AvailableQTY"] = max(0, row["TotalQty"] - row["Reserve"])
            result.append(row)

        logger.info(f"[BM] bulk_by_loc: {len(result)} SKUs únicos tras merge LOC47+LOC68")
        return result
