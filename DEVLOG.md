...
### Verificación
Local: `py -m py_compile` limpio, servidor local (`py -m uvicorn app.main:app --port 8004`) arrancó sin errores ("Application startup complete", sin tracebacks) con el loop registrado -- sin `MM_URL`/`MM_BOT_TOKEN` en local, el loop hace no-op silencioso cada ciclo (comportamiento esperado, `get_channel_posts` regresa `[]` sin lanzar).

**Producción, confirmada con datos reales de la API de Mattermost (no simulado)**: se publicó un post de prueba con identidad ajena al bot (`prueba enterado instantaneo -- ignorar`, post `btuqckt8zf843mdq9saaowfppr`, `create_at=1789109229389` → 2026-09-10 20:47:09.389 UTC, root post). El bot respondió en el hilo correcto (`root_id=btuqckt8zf843mdq9saaowfppr`) con el texto fijo "Enterado, lo estamos revisando." (post `nz747g9gnjfg9fur7up6wf68so`, `create_at=1789109231725` → 20:47:11.725 UTC). **Delta real: 2.34 segundos** (dentro del ciclo de 30s del loop) -- bien por debajo del objetivo de <60s. De paso se confirmó que `w5yspgpmf3bd5kyiweoqd9ikfa` (username `ecomops-agent`) es en efecto el `user_id` real del bot -- validado contra la API, no solo asumido.

Nota sobre el proceso de verificación: el primer intento de monitoreo automático (script de polling en background) dio un falso positivo -- su set de "posts ya vistos" se armó con un fetch de `limit=5` que no alcanzó a incluir el post raíz de un hilo viejo; al reconsultar con `limit=20` ese post raíz pareció "nuevo" y se emparejó por error con una respuesta del bot de hace ~3.7h. Se detectó por el delta absurdo (13,422s) antes de reportarlo, se corrigió el script (corte por timestamp real del servidor en vez de membresía de IDs) y se verificó el resultado final leyendo directo la respuesta cruda de `/api/diag/mattermost-posts`, no del script.
