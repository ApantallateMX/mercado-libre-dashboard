...
**AMPLIACIÓN 2026-08-20 (misma sesión, 2da vuelta): la directiva se cerró al 100% — ya NO queda ninguna excepción "pendiente".** `_update_bm_master_for_category` dejó de filtrar por "SKU conocido" (antes descartaba cualquier fila de BM cuyo SKU no estuviera ya en `ml_listings`/`amazon_listings`, lo cual hacía IMPOSIBLE detectar gaps reales — un producto nunca publicado nunca está en "conocidos"). Ahora hace UPSERT de TODA fila que BM devuelva por categoría (con `COALESCE(NULLIF(...))` para no pisar datos buenos con vacíos) — `bm_sku_master` es hoy un espejo COMPLETO del catálogo vendible de BM, no solo de lo que ya se publicó. Migradas a leer el maestro (cero llamadas nuevas a BM, incluyendo el pendiente de `amazon_lanzar.py:299` que quedaba abierto arriba): gap scan ML + Amazon (las 3 vías), `/api/bm/launch-opportunities`, `/products/sin-bm`, Sync Stock manual (circuit breaker por frescura de `bm_sku_master` en vez de ping a BM), Sync Var/`sync-variation-stocks`, `bm-cost`, modal de deals, SKU Compare/Research, contexto IA de reclamos/preguntas, `retail-ph-batch`. El sync diario de catálogo (`_sync_bm_product_catalog`, ConfColumns, 3am, 1x/día) ahora también completa `category`/`upc`/`image_url` para SKUs con 0 stock (antes esos 3 campos solo los llenaba el loop de categorías, que no toca SKUs sin stock). Quedan ~15-20 endpoints `/api/diag/*` históricos de investigaciones puntuales YA CERRADAS, desactivados con `410` (no borrados) en vez de migrados, porque su función era probar el comportamiento crudo de BM (no tiene sentido "migrarlos a leer el maestro"). Únicos que siguen llamando BM en vivo, con justificación explícita: `_sync_bm_product_catalog` (writer, no lector — 1x/día, off-peak), `_check_bm_health`/`_bm_health_loop` (ping cada 2 min a `/User/Index`, señal de "BM alcanzable" distinta a "datos frescos" — el 5to hallazgo de abajo es la prueba de por qué esta distinción importa de verdad), `/api/planning/production-kpis` (KPIs de Operaciones, dominio sin maestro equivalente). Ver `.claude/memory/project_bm_call_consolidation_2026-08-20.md` para el detalle completo de la 1ra vuelta y `project_ultimo_movimiento.md` (entrada 2026-08-20) para la 2da.

**Hallazgo importante (mismo día, 2da vuelta): "BM Disponible" mostrando avail+reserve NO es un bug de agregación — es staleness de categorías longtail.** Jovan reportó SNWM000001 mostrando "3899" cuando BM real es Available=3383/Reserve=516/NotSellable=8621 (3383+516=3899, coincidencia real, no bug). Nuevo diag `GET /api/diag/raw-category-rows` (llama `get_bulk_stock(category_id=...)`, el MISMO endpoint del loop, y devuelve la fila CRUDA sin agregar) confirmó que BM ya devuelve `AvailableQTY:3383, Reserve:516` correctos, y que `_bulk_stock_rows_to_master_fields` suma bien (`avail_total += AvailableQTY`, nunca `+Reserve`). La causa real: "Wall Mounts" es categoría longtail (no top-5 por ventas) — el dato que Jovan vio era de un ciclo previo a que BM registrara esa reserva. Forzar el refresh de la categoría lo corrigió de inmediato. **Decisión de Jovan: bajar `_CONF_COLUMNS_LONGTAIL_INTERVAL_S` de 4h a 2h** (top-5 sigue en 15 min) + full-resync manual de las 59 categorías corrido en producción el mismo día para corregir todo de inmediato. Antes de reportar "BM Disponible" como mal calculado, usar `raw-category-rows` para comparar la fila cruda contra lo guardado — casi siempre es esto, no un bug de suma.

---

## 🏷️ NOMBRES OFICIALES DE LOS 2 ENDPOINTS (Jovan, 2026-08-20)

Para comunicarse con Alberto/BinManager sobre estos 2 endpoints, usar SIEMPRE estos nombres (los definió Jovan explícitamente):

- **"Maestro"** = `Get_GlobalStock_InventoryBySKU` (CONCEPTID=1, LOCATIONID=47,62,68) — el que bajamos por categoría (loop de categorías, cada 15min/2h), nuestra base real de stock vendible, escribe en `bm_sku_master`.
- **"Alumno"** = `ConfColumns_Conditions_Excel` — el catálogo diario (1x/día, 3am), solo identidad (título/marca/modelo/categoría/UPC/imagen/costo/retail), NUNCA cantidades de stock.
...
**⚠️ CORRECCIÓN 2026-08-20 — "0 filas" para una categoría completa NO es automáticamente sinónimo de bug/bloqueo de BM.** BM esconde/oculta un SKU de la respuesta cuando su stock vendible es 0 (no lo devuelve con `AvailableQTY:0`, simplemente no aparece) — Jovan lo confirmó filtrando "Microphones-JLab" con los filtros vendibles reales en la propia UI de BM y obtuvo "Records: 100/0" limpio (sin error, sin pantalla trabada) -- señal de que la consulta corrió bien y de verdad no hay nada vendible en esa categoría ahora, no de un 500/NullReferenceException como el incidente del 2026-08-19/20 (Televisions/Headphones-JLab, confirmado con status=500 real en logs de Railway). **No asumas "bug" solo por ver 0 filas** -- antes de reportarlo como bloqueo, comparar con logs de Railway (`deploymentLogs`, buscar el status HTTP real) o con el mismo filtro vendible en la UI de BM. Para un SKU individual que no aparece dentro de una categoría que SÍ trajo otras filas, la regla sigue siendo simple y ya funciona bien: ausencia = 0 confirmado. El caso más difícil (categoría COMPLETA en 0 filas, sin saber si es bug o real) sigue sin resolverse de raíz -- ver [[project_bm_call_consolidation_2026-08-20]] para la propuesta en evaluación (confirmar cero real solo tras 2-3 ciclos consecutivos en 0, mismo criterio que el auto-halt de top-5).
